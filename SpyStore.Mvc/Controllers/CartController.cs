...
namespace SpyStore.Mvc.Controllers
{
    ...
    public class CartController : BaseController
    {
            ...
            var viewModel = new CartViewModel
                {
                    ...
                    CartRecords = mapper.Map<IList<CartRecordViewModel>>
                        (cartWithCustomerInfo)
                };
        ...
        public async Task<IActionResult> AddToCart(
            int productId, AddToCartViewModel item)
        {
            if (!ModelState.IsValid) return View(item);
            try
            {
                await _serviceWrapper.AddToCartAsync(
                ViewBag.CustomerId, productId, item.Quantity);
            }
            catch (Exception)
            {
        ...
        public async Task<IActionResult> Update(int id, CartRecordViewModel record)
        {
            ...
            var item = await _serviceWrapper
                .UpdateShoppingCartRecordAsync(id, cartRecord);
            if (item == null)
            {
